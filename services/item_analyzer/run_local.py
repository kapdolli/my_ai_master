...
인자로 넘긴 창업 아이템 후보 하나를 분석해
1인·비전문가·부업 관점의 실행 가능성 + 국내/해외 사례 + 종합 판단 리포트를 만들고
텔레그램으로 발송한다.

Usage:
    python run_local.py "AI 튜터 챗봇 — 초등 저학년 국어·수학 개별 지도"
    python run_local.py "AI 튜터" --dry-run          # 텔레그램 발송 생략
    python run_local.py --stdin                       # stdin에서 아이템 읽기
    python run_local.py --show-cmd                    # 실행할 claude 명령만 출력

환경변수:
    CLAUDE_BIN            claude CLI 실행 파일 경로 (기본 "claude")
    ITEM_ANALYZER_MODEL   예: sonnet, opus, claude-sonnet-4-6 (미지정 시 subscription 기본)
    ITEM_ANALYZER_TIMEOUT 초 단위 (기본 900 = 15분)
    ITEM_ANALYZER_TOOLS      쉼표 구분 허용 툴 (기본 "WebSearch,WebFetch")
"""
...
MODEL = os.environ.get("ITEM_ANALYZER_MODEL", "").strip()
TIMEOUT = int(os.environ.get("ITEM_ANALYZER_TIMEOUT", "900"))
TOOLS = os.environ.get("ITEM_ANALYZER_TOOLS", "WebSearch,WebFetch").strip()
TELEGRAM_CHUNK = 3800
...
def build_prompt(item: str, today_kst: datetime.date) -> str:
    body = PROMPT_FILE.read_text(encoding="utf-8")
    header = (
        f"분석 대상 아이템: {item}\n"
        f"오늘 날짜(KST): {today_kst.isoformat()}\n\n"
        "아래는 아이템분석가의 실행 사양이다. "
        "STEP 1~4를 순서대로 수행하고, 마지막에 정의된 리포트 포맷 그대로 최종 응답 텍스트로 출력하라. "
...
def slugify(text: str, maxlen: int = 40) -> str:
    """로그 파일명에 쓸 안전한 slug — 한글은 그대로, 공백은 _, 특수문자 제거."""
    text = text.strip()
    text = re.sub(r"[\\/:*?\"<>|]+", "", text)  # Windows 파일명 금지 문자
    text = re.sub(r"\s+", "_", text)
    return text[:maxlen] or "item"


def write_log(item: str, now_kst: datetime.datetime, report: str, status: str) -> Path:
    LOG_DIR.mkdir(exist_ok=True)
    stamp = now_kst.strftime("%Y-%m-%d_%H%M%S")
    log_path = LOG_DIR / f"{stamp}_{slugify(item)}.md"
    log_path.write_text(
...
def main() -> int:
    ...
    parser.add_argument(
        "item",
        nargs="?",
        help="분석 대상 아이템 (이름 + 짧은 설명). --stdin 사용 시 생략",
    )
    parser.add_argument("--dry-run", action="store_true", help="텔레그램 발송 생략")
    parser.add_argument("--stdin", action="store_true", help="stdin에서 아이템 읽기")
    parser.add_argument(
        "--show-cmd",
        action="store_true",
    ...
    item = sys.stdin.read().strip() if args.stdin else (args.item or "").strip()
    if not item:
        parser.error("아이템 문자열이 비었다. 인자로 넘기거나 --stdin 사용")

    now = datetime.datetime.now(KST)
    print(f"[start] {now.date()} item={item!r} model={MODEL or '(default)'} dry_run={args.dry_run}")

    prompt = build_prompt(item, now.date())
    report = generate_report(prompt)
    if not report:
        write_log(item, now, "(빈 응답)", "empty")
        print("[error] 리포트 본문이 비었다", file=sys.stderr)
        return 2

    log_path = write_log(item, now, report, "generated")
