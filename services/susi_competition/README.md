...
2027학년도 수시 원서접수 기간 동안 **경쟁률이 낮은 학과**를 주기적으로 수집해, 상위 50위권 대학의 농어촌·논술·기회균형 전형만 추려 웹 페이지로 배포하는 서비스.

원서 마감 직전까지 경쟁률이 계속 움직이므로, "지금 어디가 비어 있는가"를 1시간마다 갱신해 보여주는 것이 목적이다.
...
| 실행 주기 | 1시간 간격 (작업 스케줄러 `SusiRefresh`) |
...
```
[작업 스케줄러 SusiRefresh] ──1시간 간격──▶ refresh_all.ps1 (로컬, 국내 IP)
                                                │
                     ┌──────────────────────────┼──────────────────────────┐
                     ▼                          ▼                          ▼
        [1] find_low_competition.py   [3] filter_top50.py      [4] build_html.py
            전체 데이터 수집               상위50 대학 필터            단일 HTML 생성
        [2] 농특/논술 <2.0 필터            + 전형세부유형 태깅         + 관심학과 상단 고정
                                                │
                                                ▼
...
| [`filter_top50.py`](filter_top50.py) | 대학 순위표·제외규칙 적용, 전형세부유형(농어촌_교과/종합/일반, 논술, 기회균형) 태깅 |
...
| `susi_2027_top50_nong_nonsul.csv` | [3단계] 상위50 대학 필터 결과 (페이지 본문 테이블 소스) |
...
### 3. 작업 스케줄러 등록 (`SusiRefresh`, 1시간 간격)
