...
| 계열 | 공학·자연·의약보건·농생명·사회·인문·교육·예체능·융합자유·기타. 단과대는 45%가 비어 있고 표기도 115종이라, `filter_top50.py` 의 `classify_kyelyeol()` 이 **학과명을 먼저 보고 못 정하면 단과대**로 한 번 더 보아 분류한다 (미분류 0.1%) |
| 마감 | 대학별 원서 마감시각 칩 (이른 순). 2027 수시는 **9/11 18:00 이 1,359행, 9/11 17:00 이 327행**으로 대부분을 차지한다. 이미 지난 시각은 칩에 취소선 |
| 대학군 | `전체` / `★ 주요대학만`(`major_univs.txt` 지정) / `그 외 대학만`. **첫 진입 기본값은 `★ 주요대학만`** (`build_html.py` 의 `DEFAULT_MAJOR`). `초기화` 도 여기로 돌아온다. 저장된 선택이 있으면 그쪽이 우선 |
...
★주요대학 · 경쟁률 <2.0 · 모집 3명 이상 중에서 **깜깜이 구간**이 긴 순 →
경쟁률 낮은 순으로 5개를 자동으로 뽑는다. 필터와 무관하게 항상 전체 기준이다.

**깜깜이 구간 = 접수마감 − 경쟁률마감.** 경쟁률 공개가 끊긴 뒤 마감까지 남는 시간으로,
길수록 지원자들이 막판 눈치작전을 할 수 없어 지금 낮은 경쟁률이 유지될 가능성이 크다.
반대로 마감 직전까지 공개하는 대학은 미달 학과에 막판 쏠림이 일어난다.
3시간 이상이면 초록, 그 미만이면 주황 배지.

같은 학과가 여러 캠퍼스로 중복 등록된 경우(경기대 서울/수원 등)는 하나로 묶고
캠퍼스 표기를 뺀 대학명으로 보여준다. 계산은 `blackout_minutes()` 에 있다.

> 이 순위는 정보 구조상의 경향이지 예측이 아니다. 실제로는 지원 자격(농어촌·기회균형),
> 수능최저, 학과 선호도가 더 크게 작용한다.
...
| 경쟁률기준 | **지금 보고 있는 경쟁률이 언제 집계된 것인지.** 대학마다 갱신 주기가 달라(10분/1시간/하루 2회) 같은 화면 안에서도 신선도가 섞인다 |

- 경쟁률마감이 지나면 **회색 + 취소선**이 된다 — 그 대학 숫자는 더 오르지 않는다.
  (배지를 쓰면 칸 폭이 28px 늘어 마감이 시작되는 순간 전부 넘쳐서 뺐다)
- 파싱에 실패하면 **`미상`(주황)** 으로 두고 추측하지 않는다. 현재 약 73개 중 59개 대학 파싱.
- 셀에 마우스를 올리면 **그 대학 경쟁률의 기준시각 + 공지 원문**이 뜬다.
- 날짜 칸(접수마감·경쟁률마감·기준)은 자료가 전부 9월이라 월을 떼고 **`일-시:분`**
  형태로 통일했다 (`11-18:00`, 기준 `10-10:50`). 전체 값은 툴팁에 있다.
